...
<body>
                    ...
                    <div class="modal fade text-left modal-borderless" id="modal_detail" tabindex="-1"
                        ...
                        <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
                            <div class="modal-content overflow">
                                ...
                                <div class="modal-body">
                                    <form id="form_detail">
                                        <table id="tabelDetail" class="table dt-table-hover wrapped"
                                            style="width:100%">
                                            <thead>
                                                <th>Fasum</th>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            initializeSelect2('select[name=\'pic_fasum[]\']', "{{ route('pelaporanadmin.getDataStaff') }}",
                'Pilih Staff');
            initializeSelect2('select[name=\'fasum[]\']', "{{ route('pelaporanadmin.getDataFasum') }}",
                'Pilih Fasum');
            let map;
            let marker;
            ...
            $("#btnTambahBaris").click(function() {
                ...
                rowData.push(
                    '<select required class="form-control form-control-user pic_fasum" id="pic_fasum[' +
                    i +
                ...
                initializeSelect2('#user', "{{ route('pelaporanadmin.getDataUser') }}", 'Pilih User');
                initializeSelect2('#pic_utama', "{{ route('pelaporanadmin.getDataStaff') }}",
                    'Pilih Staff');
                initializeSelect2('select[name=\'pic_fasum[]\']',
                    "{{ route('pelaporanadmin.getDataStaff') }}", 'Pilih Staff');
                initializeSelect2('select[name=\'fasum[]\']', "{{ route('pelaporanadmin.getDataFasum') }}",
                    'Pilih Fasum');
                i++;
            });
        ...
        function save(id) {
            ...
            // blom bisa lol tunggu besok lusa ya
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(key, item) {
                        ...
                        rowData.push('<select class="form-control form-control-user" id="pic_fasum[' +
                            key + ']" style="width:150px" name="pic_fasum[]"><option value="' + item
                            .id_staff_detail + '">' + item.nama_staff_detail + '</option></select>');
                        rowData.push(
                        ...
                        initializeSelect2('#user', "{{ route('pelaporanadmin.getDataUser') }}",
                            'Pilih User');
                        initializeSelect2('#pic_utama', "{{ route('pelaporanadmin.getDataStaff') }}",
                            'Pilih Staff');
                        initializeSelect2('select[name=\'pic_fasum[]\']',
                        ...
                        initializeSelect2('select[name=\'fasum[]\']',
                            "{{ route('pelaporanadmin.getDataFasum') }}", 'Pilih Fasum');
                    });
