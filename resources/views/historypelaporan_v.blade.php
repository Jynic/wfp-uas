...
<body>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            initFlatPic();
            generateSTCode();
            initializeSelect2('select[name=\'pic_fasum[]\']', "{{ route('historypelaporan.getDataStaff') }}",
                'Pilih Staff');
            initializeSelect2('select[name=\'fasum[]\']', "{{ route('historypelaporan.getDataFasum') }}",
                'Pilih Fasum');
            let map;
            let marker;
            var i = 10;
            ...
            $("#btnTambahBaris").click(function() {
                var rowData = [];
                rowData.push('<select class="form-control form-control-user fasum" id="fasum[' + i +
                ...
                initializeSelect2('select[name=\'pic_fasum[]\']',
                    "{{ route('historypelaporan.getDataStaff') }}", 'Pilih Staff');
                initializeSelect2('select[name=\'fasum[]\']',
                    "{{ route('historypelaporan.getDataFasum') }}", 'Pilih Fasum');
                i++;
            });
        });
        ...
        function detail(id) {
            $.ajax({
                type: 'POST',
                url: "{{ route('historypelaporan.detail') }}",
                data: {
        ...
        function save(id) {
            ...
            if (id == 0) {
                var url = "{{ route('historypelaporan.simpan') }}";
            } else {
                var url = "{{ route('historypelaporan.update') }}";
            }
        ...
        function edit(id) {
            $.ajax({
                type: 'POST',
                url: "{{ route('historypelaporan.edit') }}",
                data: {
                    '_token': '<?php echo csrf_token(); ?>',
                    'id': id
                },
        ...
        function hapus(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ Route('historypelaporan.hapus') }}",
                        type: "POST",
                        data: {
                            id: id,
                            _token: '<?php echo csrf_token(); ?>'
